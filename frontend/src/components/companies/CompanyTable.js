...
function EnhancedTableToolbar(props) {
  ...
  const deleteCompanies=async()=>{
    ...
    await axios.delete('http://localhost:80/company/delete-company',{
      data:{company_list:selected}
    },config)
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-slide-description">
            Are u sure delete selected companies and releational products ? 
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleClose}>Disagree</Button>
          <Button onClick={deleteCompanies}>Agree</Button>
        </DialogActions>
      </Dialog>
        </Tooltip>
      ) : (
...
export default function EnhancedTable({change}) {
  ...
  const saveData=async()=>{
    const updatedData = {
      id: current._id,
      name:current.name,
      legalNumber:current.legalNumber,
      incorporationCountry:current.incorporationCountry,
      website:current.website
      // Diğer güncellenmesi gereken alanları buraya ekleyin
    };
    await axios.put('http://localhost:80/company/update-company', updatedData)
    .then((response) => {
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="right">{row.legalNumber}</TableCell>
                    <TableCell align="right">{row.incorporationCountry}</TableCell>
...
<TextField
  ...
  value={current.legalnumber||""} // Burada value özelliğini kullanın
  onChange={(e) => setCurrent({ ...current, legalnumber: e.target.value })} // current.legalnumber'ı güncelleyin
  style={{ marginTop: '10px' }}
  size="small"
  variant='outlined'
/>
<TextField
  label="Incorporation Country"
  id="outlined-size-small"
  value={current.inc||""} // Burada value özelliğini kullanın
  onChange={(e) => setCurrent({ ...current, inc: e.target.value })} // current.inc'i güncelleyin
  style={{ marginTop: '10px' }}
  size="small"
  variant='outlined'
/>
<TextField
  ...
  id="outlined-size-small"
  value={current.website||""} // Burada value özelliğini kullanın
  onChange={(e) => setCurrent({ ...current, website: e.target.value })} // current.website'i güncelleyin
  style={{ marginTop: '10px' }}
  size="small"
  variant='outlined'
/>
